...
def visa(text):
    t=norm(text)
    if any(x in t for x in ['visa sponsorship available','sponsorship available','will sponsor','visa sponsorship','work permit sponsorship']):return {'status':'explicit','confidence':.95}
    if any(x in t for x in ['relocation assistance','relocation support','relocation package']):return {'status':'likely','confidence':.72}
...
def score_job(j,p):
    ...
    s=max(0,min(100,s)); decision='APPLY' if s>=80 else 'REVIEW' if s>=65 else 'MAYBE' if s>=50 else 'SKIP'
    return {**j,'dedupe_key':key(j),'last_seen':datetime.now(timezone.utc).isoformat(),'experience':exp,'visa':vs,'match':{'score':s,'confidence':.75 if primary else .55,'reasons':reasons},'decision':decision}
